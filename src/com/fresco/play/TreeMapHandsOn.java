...
public class TreeMapHandsOn {
			...
			if (Integer.parseInt(playerDetails[0]) == 1) {
				if (nameWithNumberOfMatchesMap.size() == 0 && nameWithTotalScore.size() == 0) {
					...
					if (nameWithNumberOfMatchesMap.get(playerDetails[1]) != null)
						nameWithNumberOfMatchesMap.put(playerDetails[1],
								nameWithNumberOfMatchesMap.get(playerDetails[1]) + 1);
					else
						nameWithNumberOfMatchesMap.put(playerDetails[1], 1);
					if (nameWithTotalScore.get(playerDetails[1]) != null) {
						nameWithTotalScore.put(playerDetails[1],
								nameWithTotalScore.get(playerDetails[1]) + Integer.parseInt(playerDetails[2]));
					} else {
						nameWithTotalScore.put(playerDetails[1], Integer.parseInt(playerDetails[2]));
					}
				}
	...
	public static void main(String[] args) {
		...
		exp.put(2, "Rohit");
		exp.put(4, "Pandya");
		TreeMap<Integer, String> actual = createPlayerPositionMap(
				"3,Dhoni,100|1,Virat,50|5,Jadeja,90|2,Rohit,70|4,Pandya,30");
	}
}
